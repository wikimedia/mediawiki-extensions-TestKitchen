...
class PageBeacon implements BeforePageDisplayHook {
	private const CONSTRUCTOR_OPTIONS = [
		...
		'TestKitchenEnableHeadPixel',
		'TestKitchenHeadPixelMetric'

		// TestKitchen doesn't and can't depend on WikimediaEvents as that would be
		// circular. We'll test for WMEStatsdBaseUri in the hook.
		// WMEStatsdBaseUri
	];

	public const TEST_KITCHEN_EXPERIMENT_NAME_FOR_HEAD_PIXEL = 'test-kitchen-mw-module-loaded-v2';
	...
	public function onBeforePageDisplay( $out, $skin ): void {
		if (
			!$this->options->get( 'TestKitchenEnableExperiments' ) ||
			!$this->options->get( 'TestKitchenEnableHeadPixel' ) ||
			!$this->options->get( 'TestKitchenHeadPixelMetric' ) ||
		...
		$experiment = $this->experimentManager->getExperiment( self::TEST_KITCHEN_EXPERIMENT_NAME_FOR_HEAD_PIXEL );
		...
		if ( !$out->isPrintable() && $out->getActionName() === 'view' ) {
			...
			$metric = $this->options->get( 'TestKitchenHeadPixelMetric' );

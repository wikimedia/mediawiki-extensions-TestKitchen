...
class ConfigsFetcher {
	...
	/**
	 * Fetch instrument configs from Test Kitchen and update the backing store if they have changed.
	 *
	 * @internal
	 */
	...
	/**
	 * Fetch experiment configs from Test Kitchen and update the backing store if they have changed.
	 *
	 * @internal
	 */
	...
	private function fetchConfigs( int $type ): Status {
		$isExperiment = $type === self::EXPERIMENT;
		$endpoint = $isExperiment
			? self::TESTKITCHEN_API_EXPERIMENTS_ENDPOINT
			: self::TESTKITCHEN_API_INSTRUMENTS_ENDPOINT;
		$url = $this->options->get( 'TestKitchenInstrumentConfiguratorBaseUrl' ) . $endpoint;
		...
		if ( !$responseStatusCode < 200 && $responseStatusCode >= 300 ) {
			$status->fatal( 'testkitchen-non-successful-response' );
		}

		if ( !$responseBody ) {
			$status->fatal( 'testkitchen-api-empty-response-body' );
		} else {
	...
	/**
	 * Post-processes the result of successful request to Test Kitchen by:
	 *
	 ...
	 *
	 * @param array $result An array of configs retrieved from Test Kitchen
	 * @param int $type Either `self::INSTRUMENT` or `self::EXPERIMENT`
	...
	protected function processConfigs( array $result, int $type ): array {
		...
		foreach ( $result as $config ) {
			...
			try {
				...
				$this->logger->error(
					'Could not parse start/end for {experiment}. Test Kitchen API might be emitting invalid dates',
					[
						'experiment' => $name,
						'exception' => $e,
					]
				...
				$this->logger->error(
					'{experiment} has no groups. Test Kitchen API might be emitting invalid configs',
					[
						'experiment' => $name,
					]
				);
	...
	/**
	 * Increment success/failure of Test Kitchen API requests.
	 *
	 * @param string $label
	 */
	private function incrementApiRequestsTotal( string $label ): void {
		$this->statsFactory->getCounter( 'test_kitchen_api_requests_total' )
			->setLabel( 'status', $label )
			->increment();
	}

	/**
	 * Record duration of Test Kitchen API requests.
	 *
	 * @param float $startTime
	 */
	private function logApiRequestDuration( float $startTime ): void {
		$this->statsFactory->getTiming( 'test_kitchen_api_request_duration_seconds' )
...
class_alias( ConfigsFetcher::class, 'MediaWiki\\Extension\\TestKitchen\\InstrumentConfigsFetcher' );

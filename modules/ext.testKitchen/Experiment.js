 ...
 *
 *  Note well that this class should be constructed using `mw.testKitchen.getExperiment()` instead, e.g.
 *
 *  ```
 *  const experiment = mw.testKitchen.getExperiment( 'my-awesome-experiment' );
 *  ```
 * @hideconstructor
 * @package
 */
class Experiment {
	 ...
	 * `ext.wikimediaEvents.testKitchen` ResourceLoader module][0] by proxying to {@link Experiment#send}.
	 * Calling this outside of Test Kitchen is not supported.
	 *
	...
	setStream( streamName ) {
		if ( !this.metricsClient.getStreamConfig( streamName ) ) {
			...
			console.warn(
				'%s: The stream %s isn\'t registered. Events will not be sent for this experiment. Have you added %s to $wgTestKitchenExperimentStreamNames?',
				this.name,
				streamName,
				streamName
			);
